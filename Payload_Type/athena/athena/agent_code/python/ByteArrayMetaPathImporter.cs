...
module, or raises ResourceImportError if it wasn't found."
        ...
        public object load_module(CodeContext /*!*/ context, string fullname)
        {
            ...
            IDictionary<object, object> modules = (IDictionary<object, object>)typeof(PythonContext).GetProperty("SystemStateModules", BindingFlags.Instance | BindingFlags.NonPublic).GetValue(langContext);
        ...
        private class PackedByteLoader
        {
            private readonly byte[] _zipBytes;
            private const int MaxPathLen = 256;

            public PackedByteLoader(byte[] zipBytes)
            {
                _zipBytes = zipBytes;
            }
            ...
            {
                return new MemoryStream(this._zipBytes);
            }
